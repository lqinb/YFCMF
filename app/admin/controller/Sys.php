...
class Sys extends Base {
	...
	public function excel_export(){
		$list  = Db::query('SHOW TABLE STATUS FROM '.config('database.database').' where name = "yf_result"');
//		$list  = Db::query('SHOW TABLE STATUS FROM '.config('database.database').'where name = yf_excel');
...
//			print_r($sql);exit;
			...
			//			根据上传文件的命名 去除.xls 获得存储的table名
			$tablename = substr($_FILES ['file_stu'] ['name'],0,-4);
...
//					print_r($data);exit;
//					$data['rijun'] = floatval($data['rijun']);
...
//				'zhihang' => '桥头支行',
//			);
//			$databank = Db::name ($tablename)->where($querydata)->select();
//			print_r($databank);exit;
	...
	public function excel_runprocess(){

//		$arr = similar_text('付庄支行','美鑫支行');
//		$arr = str_compare('营业部','营销一部');
//		$arr = 0;
//		if(strstr('美鑫','美鑫支行') !== false){
//			$arr = 1;
//		}
//		print_r(strstr('111美鑫支行','美鑫'));
//		print_r($arr);

		$sql = 'TRUNCATE ' .config('database.prefix').'result';
//			print_r($sql);exit;
		Db::execute($sql);
//exit;
...
//		print_r($bank);exit;
		...
		$real = Db::name ('real')->select();
//		print_r($real);exit;
		$result = Db::table('yf_real')
			->alias('a')
//			->join('yf_bank b','a.zhihang = b.zhihang')
			->join('yf_bank b','a.yuangong = b.yuangong')
			->select();
//		print_r($result);exit;

		$data=array();
		foreach ($real as $reak => $reav) {
		foreach ( $result as $resk => $resv ) {
//			foreach ($real as $reak => $reav) {
//				print_r($reav);exit;
				if($reav['yuangong'] == $resv['yuangong'] && str_compare($reav['zhihang'],$resv['zhihang'])){
					unset($resv['id']);
					$data[$reak]= $resv;
				}
//				else{
...
//				print_r($data);exit;
		...
		foreach($data as $k => $v){
			$result1 = Db::name ('result')->insert($v);
		}
...
//
//		);
//		$result = Db::name ('bank')->where($data)->delete();
//		print_r('导入数据库成功');
//		exit;
//		$this->success ('导入数据库成功',url('excel_import'));
	}
	...
	/*
	 * 导入系统导出数据 默认处理后存入bank表单中
	 * @author rainfer <81818832@qq.com>
	 */
	public function excel_runimportbank(){
...
//			print_r($res);exit;
			$titles=array('zhihang','yuangong','rijun');
//			print_r($titles);exit;
			foreach ( $res as $k => $v ){
				if ($k != 1){
					$data=array();
...
//			print_r($databank);exit;
	...
	/*
	 * 导入发送数据 默认处理后存入real表单中
	 * @author rainfer <81818832@qq.com>
	 */
	public function excel_runimportreal(){
...
//			print_r($res);exit;
			$titles=array('zhihang','yuangong','kaohejishu','rijunzengzhang');
//			print_r($titles);exit;
			foreach ( $res as $k => $v ){
				if ($k != 1){
					$data=array();
