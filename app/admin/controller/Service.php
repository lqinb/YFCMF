...
//        $where=$diyflag?"FIND_IN_SET('$diyflag',news_flag)":'';
        $news=Db::name('News')->alias("a")->field('a.*,b.*,c.menu_name')->join(config('database.prefix').'member_list b','a.news_auto =b.member_list_id')->join(config('database.prefix').'menu c','a.news_columnid =c.id')->where($map)->order('news_time desc')->paginate(config('paginate.list_rows'),false,['query'=>get_query()]);
        $show = $news->render();
...
//        $this->assign('diyflag',$diyflag_list);
        ...
        if(request()->isAjax()){
            return $this->fetch('Service/ajax_news_list');
        }else{
            return $this->fetch('Service/news_list');
        }
    }
    ...
    public function news_add(){
        ...
        return $this->fetch('Service/news_add');
    }
    //添加操作
...
//		print_r($sl_data);exit;
    ...
    public function news_edit(){
        ...
        return $this->fetch('Service/news_edit');
    }
    ...
    public function news_back(){
        ...
        $map['news_columnid'] = array('in',array(31,32));
        $where=$diyflag?"FIND_IN_SET('$diyflag',news_flag)":'';
        $news=Db::name('News')
            ->alias("a")
        ...
        $this->assign('news',$news);
        if(request()->isAjax()){
            return $this->fetch('Service/ajax_news_back');
        }else{
            return $this->fetch('Service/news_back');
        }
    }
    //回收站还原文章
